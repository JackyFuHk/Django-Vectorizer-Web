let input_point_list = [];
let input_point_label = [];
let image_name = "";
let input_image = "";
let index = 0;
let is_auth = false;
document.addEventListener('DOMContentLoaded', function () {
    ...
    document.getElementById('clickableImage').addEventListener('click', function (event) {
        ...
        // 将星星添加到覆盖层容器中
        document.getElementById('star-overlay').appendChild(star);
...
function do_sam(x, y) {
    nowpoint = [x, y]
    input_point_list.push(nowpoint)
    input_point_label.push(1)
    // console.log(input_point_list, input_point_label, input_image);
    fetch(`/sam/`, {
        method: 'POST',
        headers: {
        ...
        body: JSON.stringify({
            "logo_url": input_image,
            "input_point_list": input_point_list,
            "input_label_list": input_point_label,
            "name": image_name
        }),
    })
        .then(response => response.json())
        .then(data => {
            var base64String = data['data']['data']
            overlay.style.display = 'none'

            // addicon(base64String, image_name, index)
            index = index + 1
            ...
            a.href = url;
            a.download = image_name + '.png'; // 根据需要设置文件扩展名
            ...
            for (let i = 0; i < uploadFiles.length && i < maxFiles; i++) {
                ...
                if (file.type.startsWith("image/")) {
                    ...
                    try {
                        ...
                        document.getElementById('clickableImage').src = data["user_png"];
                        input_image = data["user_png"];
                        // console.log('Success for file ' + (i + 1) + ':', data);
