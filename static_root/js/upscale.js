    ...
    if (imageHistoryData !== undefined && (typeof imageHistoryData === 'string' ? imageHistoryData.trim() !== '' : imageHistoryData.length > 0)) {
        ...
        setTimeout(function () {
            for (ii = 0; ii < imageHistoryData.length; ii++) {
                ...
                document.getElementById('status' + ii).innerHTML = "放大完成";
    ...
    imagesInRow.forEach(function (image) {
        image.addEventListener('click', function () {
            ...
            setTimeout(function () {
                ...
                rightImagesrc.src = image.src.replace('.png', '_result.png');
            }, 1200);
        });
    });
...
function addicon(imgsrc, imgname, index) {
    ...
    // 设置 select 元素的属性，如 name 和 id（可选）
    selectElement.id = 'select' + index;

    // 创建第一个 option 元素，并设置其文本内容和 value 属性
    const option1 = document.createElement('option');
    option1.textContent = '放大2倍';
    option1.value = '1';

    // 将第一个 option 元素添加到 select 元素中
    selectElement.appendChild(option1);

    // 创建第二个 option 元素，并设置其文本内容和 value 属性
    const option2 = document.createElement('option');
    option2.textContent = '放大4倍';
    option2.value = '2';

    // 将第二个 option 元素添加到 select 元素中
    selectElement.appendChild(option2);

    // 将容器添加到 DOM 中，例如添加到 body 末尾
    upload_history.appendChild(selectElement);
    ...
    div.addEventListener('click', function () {
        times = document.getElementById('select' + index).value;
        transfer_svg(imgsrc, index, imgname, times)
    });
...
async function transfer_svg(imgsrc, index, imgname, times = 1) {
    is_authority();
    // 如果已经登录，可以运行。
    if (is_auth == false) {
        ...
        if (is_ok == 1) {
            ...
            document.getElementById('status' + index).innerHTML = "正在放大中...";
            document.getElementById('btn' + index).innerHTML = "正在图片放大";
            try {
